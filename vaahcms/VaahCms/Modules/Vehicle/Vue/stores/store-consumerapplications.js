...
let model_namespace = 'VaahCms\\Modules\\Vehicle\\Models\\ConsumerApplication';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/vehicle/consumerapplications";
...
export const useConsumerApplicationStore = defineStore({
    ...
    state: () => ({
        ...
        route: null,
        watch_stopper: null,
        route_prefix: 'consumerapplications.',
        view: 'large',
        show_filters: false,
        list_view_width: 12,
        form: {
    ...
    actions: {
        ...
        setViewAndWidth(route_name)
        {
            switch(route_name)
            {
                case 'consumerapplications.index':
                    this.view = 'large';
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'consumerapplications.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'consumerapplications.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'consumerapplications.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'consumerapplications.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'consumerapplications.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useConsumerApplicationStore, import.meta.hot))
}
